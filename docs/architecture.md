...
`bevy_brp_extras` 通过 `vendor/bevy_brp_extras/` 中隔离的外部兼容包接入；该目录基于 crates.io 发布包维护，不属于 Workspace member，也不是 Widgetry 功能 crate。兼容层保留上游默认 HTTP 装配，并提供由宿主自行安装 transport 的 methods-only 入口。
