...
/// 构造绑定原生窗口与显式 UI 相机的完整窗口场景，需先注册 WidgetryWindowPlugin 和 Bevy 资产、场景插件。
///
/// 原生窗口须在创建前通过 prepare_native_window 准备，保持 transparent 为 true、decorations 为 false。
/// composite_alpha_mode 必须为 PreMultiplied；Windows DX12 还须满足 prepare_native_window 文档中的渲染初始化要求。
...
/// 需先注册 WidgetryWindowPlugin 和 Bevy 资产、场景插件。原生属性自动经 prepare_native_window 准备。
