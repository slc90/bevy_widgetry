...
mod tests {
    ...
    // 连续激活每种导航并重复激活当前页，必须始终只显示目标页面。
    #[test]
    fn activation_selects_exactly_one_existing_page() {
        let mut world = World::new();
        world.add_observer(on_nav_button_activated);
        let pages = [
            GalleryPage::Button,
            GalleryPage::ComboBox,
            GalleryPage::TextField,
        ]
        .map(|page| {
            let entity = world
                .spawn((GalleryPageContent(page), Node::default()))
                .id();
            let nav = world.spawn(GalleryNavButton(page)).id();
            (page, entity, nav)
        });

        for (_, selected_entity, nav) in pages.into_iter().cycle().take(6) {
            for _ in 0..2 {
                world.trigger(Activate { entity: nav });
                for (_, entity, _) in pages {
                    let expected = if entity == selected_entity {
                        Display::Flex
                    } else {
                        Display::None
                    };
                    assert_eq!(world.get::<Node>(entity).unwrap().display, expected);
                }
            }
        }
    }
