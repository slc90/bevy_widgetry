...
能够由 Workspace 统一管理的外部 crate，其版本统一定义在 root `Cargo.toml` 的 [workspace.dependencies] 中。
...
不得因为实现方便、可能以后会用或为了引入某个小 helper function 而随意增加依赖。新增依赖必须服务于当前明确目标，并遵守 Workspace 统一版本管理。
