...
出现相应失败路径时必须记录；不得为了满足等级规则人为制造日志。

### 操作与结果

Gallery 应记录具有明确演示语义的离散操作，例如页面切换、主题切换、控件值确认、打开窗口或对话框、发起外部操作以及这些操作的明确结果。
记录操作的业务语义，不记录底层输入机制。例如记录“打开独立窗口”，而不是“收到 Pointer<Click>”。
运行期程序化操作如果具有明确演示意义，应在 Gallery 的调用处记录，并与用户操作区分。
程序初始化保持静默，不得为了区分初始化与运行期操作而给 Widgetry 库增加日志开关、静默参数或额外状态。
...
* 普通 focus 变化；
* 文本逐字符输入；
* 控件内部状态同步、布局更新及其他连续执行轨迹。

如果某个连续交互最终产生具有明确演示语义的确认、提交或选择结果，可以在该结果发生时记录一次。
日志应附着在已有 Gallery 语义处理点，不得为了日志新增轮询 system、全局输入监听或低层事件追踪。
不得因为日志改变现有异步流程、控件行为或 UI 反馈。
